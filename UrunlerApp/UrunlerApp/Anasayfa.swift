...
class Anasayfa: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        urunlerTableView.separatorColor = UIColor(white: 0.95, alpha: 1) // aradaki çizgiler görünmesin diye arkaplanla aynı renk yaptık.
    }
...
extension Anasayfa : UITableViewDelegate, UITableViewDataSource, HucreProtocol {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        hucre.backgroundColor = UIColor(white: 0.95, alpha: 1) // ( alpha 0 olursa görünmez,1 olursa görünür)
        hucre.hucreArkaplan.layer.cornerRadius = 10.0 // köşe kıvrımı
        
        hucre.selectionStyle = .none // seçilen ürünü daha koyu göstermemesi için
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) { // Satır seçimiyle ilgili işlemleri anasayfada kodladık ama hücre içinde herhangi bir işlem yapılacaksa (butona tıklama gibi) urunlerHucrede kodlanır.
        let urun = urunlerListesi[indexPath.row]
        performSegue(withIdentifier: "toDetay", sender: urun)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) { // hangi sayfaya geçiş yapacağımızı burada belirteceğiz
